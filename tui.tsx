...
function AnswerDialog(props: AnswerDialogProps) {
  ...
  const estimatedContentHeight =
    estimateMiniMessagesHeight(messages, props.state, transcriptContentWidth) +
    4;
  ...
  if (part.type === "tool") {
    const title =
      "title" in part.state && typeof part.state.title === "string"
        ...
        : part.tool;
    const label =
      title === part.tool ? `${part.tool}` : `${part.tool}: ${title}`;
    return {
      type: "tool",
      status: part.state.status,
      text: `${label} ${formatToolState(part)}`,
    };
  }
  if (part.type === "file")
...
async function startQuestion(
  ...
  const continueInMainThread = async () => {
    const answer =
      extractAssistantTextFromEntries(dialogState.entries) ||
    ...
    try {
      await api.client.session.promptAsync(
        {
          sessionID,
          parts: [
            { type: "text", text: buildContinuePrompt(question, answer) },
          ],
        },
        { throwOnError: true },
      );
      api.ui.toast({
        variant: "success",
        message: "Continued in the main thread.",
      });
      await cleanup();
    } catch (cause) {
      api.ui.toast({
...
function summarizeValue(value: unknown): string {
  if (typeof value === "string")
    return truncate(value.replace(/\s+/g, " "), 48);
